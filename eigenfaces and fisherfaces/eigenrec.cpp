...
EGRec::EGRec(string trainf, string testf)
{
    ifstream in;
...
Mat EGRec::ave_mat(vector<Mat> mat_vec)
{
    if(mat_vec.size()==0)
        return Mat();
...
Mat EGRec::norm_0_255(InputArray _src)
{
    Mat src = _src.getMat();
